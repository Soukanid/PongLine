...
export class AuthService {
  ...
  private async callUserService<T>(
    ...
    if (!response.ok) {
      const errorMessage = 
      (parsedPayload as any)?.error ||
      response.statusText ||
      'Unknown error';
      throw new Error(
        `User service error: ${errorMessage}`,
      );
    }
 ...
 async login(email: string, password: string, twoFactorCode?: string) {
    ...
    // 2. Verify password
    const isValidPassword = await bcrypt.compare(password, authUser.passwordHash)
  ...
  async validateToken(accessToken: string) {
    try {
      ...
      // Verify with User Service that profile still exists
      try {
        await this.callUserService('GET', `/api/users/${authUser.userId}/exists`)
      } catch {
